...
impl<'a> ParseContext<'a> {
    ...
    pub fn line_comment_block_vec(&self) -> Vec<String> {
        let mut result: Vec<String> = Vec::default();
        self.line_comment_block(|comment| result.push(comment.to_string()));
        result
    }
        ...
        F: FnMut(&str),
    {
        ...
        while !lines.is_empty() {
            ...
            if last_line.value().starts_with(line_comment_delimiter) {
                add_fn(&last_line.value()[line_comment_delimiter.len()..]);
            }
        }
    }
...
mod tests {
    ...
    fn line_comment_block() {
        ...
        let comments: Vec<String> = context.line_comment_block_vec();

        assert_eq!(
            comments.iter().map(|t| t.as_str()).collect::<Vec<&str>>(),
